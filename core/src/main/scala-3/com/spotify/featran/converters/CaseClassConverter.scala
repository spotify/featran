...
object CaseClassConverter {
  ...
  inline def toSpec[A <: Product](using m: Mirror.ProductOf[A], d: DefaultTransform[Double]): FeatureSpec[A] =
    featureSpec[A, m.MirroredElemTypes, m.MirroredElemLabels](0)(FeatureSpec.of[A], d)

  inline private def featureSpec[A <: Product, Elems <: Tuple, Labels <: Tuple](n: Int)(s: FeatureSpec[A], d: DefaultTransform[Double]): FeatureSpec[A] =
            ...
            case _: (label *: labels1) =>
              ...
              val spec = inline erasedValue[elem] match {
                ...
                case _: Option[Boolean] =>
                  s.optional(_.productElement(n).asInstanceOf[Option[Boolean]].map(_.asDouble))(d(name))
                case _: Option[Double] =>
                  s.optional(_.productElement(n).asInstanceOf[Option[Double]])(d(name))
                case _: Seq[Int] =>
                  s.required(_.productElement(n).asInstanceOf[Seq[Int]].map(_.toDouble))(VectorIdentity(name))
                case _: Seq[Long] =>
                  s.required(_.productElement(n).asInstanceOf[Seq[Long]].map(_.toDouble))(VectorIdentity(name))
                case _: Seq[Short] =>
                  s.required(_.productElement(n).asInstanceOf[Seq[Short]].map(_.toDouble))(VectorIdentity(name))
                case _: Seq[Boolean] =>
                  s.required(_.productElement(n).asInstanceOf[Seq[Boolean]].map(_.asDouble))(VectorIdentity(name))
                case _: Seq[Double] =>
                ...
                case _: Seq[WeightedLabel] =>
                  s.required(_.productElement(n).asInstanceOf[Seq[WeightedLabel]])(NHotWeightedEncoder(name))
                case _ => s
              }
