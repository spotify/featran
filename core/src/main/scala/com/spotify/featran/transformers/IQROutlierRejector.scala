...
import com.twitter.algebird.QTreeAggregator
 ...
 /**
  * Rejects features if they fall outside of either 1.5 times the IQR below the first quartile,
  * or [[IQROutlierRejector.DefaultFactor]] times the IQR above the third.
  *
  * The IQR or inter quartile range is the range between the first and the third quartiles.
  *
  ...
  *
  * All values are transformed to zero vectors.
  *
  * Values contained in the last bucket will report [[FeatureRejection.Outlier]] as rejection.
  *
...
object IQROutlierRejector {
    ...
    *
    * @param rejectLower whether to reject outlier based on the lower igr bound.
    * @param rejectUpper whether to reject outlier based on the upper iqr bound.
