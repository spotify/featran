...
object FeatureSpecSpec extends Properties("FeatureSpec") {
  ...
  property("required") = Prop.forAll { xs: List[Record] =>
    ...
    Prop.all(
      ...
      f.featureValues[Seq[Double], Double] == xs.map(r => Seq(r.d)))
  }
  ...
  property("optional") = Prop.forAll { xs: List[Record] =>
    ...
    Prop.all(
      f.featureNames == Seq(Seq("id")),
      f.featureValues[Seq[Double], Double] == xs.map(r => Seq(r.optD.getOrElse(0.0))))
  }
  ...
  property("array") = Prop.forAll { xs: List[Record] =>
    ...
    Prop.all(
      ...
      f.featureValues[Seq[Double], Double].toList == xs.map(r => Seq(r.d)))
  }
  ...
  property("default") = Prop.forAll { xs: List[Record] =>
    ...
    Prop.all(
      f.featureNames == Seq(Seq("id")),
      f.featureValues[Seq[Double], Double] == xs.map(r => Seq(r.optD.getOrElse(0.5))))
  }
  ...
  property("nones") = Prop.forAll { xs: List[Record] =>
    ...
    Prop.all(
      f.featureNames == Seq(Seq("id")),
      f.featureValues[Seq[Double], Double] == xs.map(r => Seq(0.5)))
  }
  ...
  property("composite") = Prop.forAll { xs: List[Record] =>
    ...
    Prop.all(
      ...
      f.featureValues[Seq[Double], Double] == xs.map(r => Seq(r.d, r.optD.getOrElse(0.5))))
  }

  property("cross") = Prop.forAll { xs: List[Record] =>
    val f = FeatureSpec.of[Record]
      .required(_.d)(Identity("id1"))
      .optional(_.optD, Some(0.5))(Identity("id2"))
      .cross("id1", "id2", (a, b) => a)
    ...
    val result = xs.map(r => Seq(r.d, r.optD.getOrElse(0.5), r.d))
    Prop.all(
  ...
  property("original") = Prop.forAll { xs: List[Record] =>
    ...
    Prop.all(
      f.featureNames == Seq(Seq("id")),
      f.featureResults[Seq[Double], Double] == xs.map(r => FeatureResult(Seq(r.d), Map.empty, r)))
  }
  ...
  property("combine") = Prop.forAll { xs: List[Record] =>
    ...
    Prop.all(
      ...
      result.featureResults[Seq[Double], Double]
        == xs.map(r => FeatureResult(Seq(r.d, r.d), Map.empty, r)))
  }
  ...
  property("missing feature in settings") = Prop.forAll { xs: List[Record] =>
    ...
    val settings = spec1.extract(xs).featureSettings
    val t = Try(spec2.extractWithSettings(xs,  settings).featureValues[Seq[Double], Double])
  ...
  property("seq") = Prop.forAll(Gen.listOfN(100, Arbitrary.arbitrary[Double])) { xs =>
    ...
    Prop.all(
      fs.extract(xs.asInstanceOf[Traversable[Double]]).featureValues[Seq[Double], Double].map(_.head) == xs,
      fs.extract(xs.asInstanceOf[Iterable[Double]]).featureValues[Seq[Double], Double].map(_.head) == xs,
      fs.extract(xs.asInstanceOf[Seq[Double]]).featureValues[Seq[Double], Double].map(_.head) == xs,
      fs.extract(xs.toIndexedSeq).featureValues[Seq[Double], Double].map(_.head) == xs,
      fs.extract(xs).featureValues[Seq[Double], Double].map(_.head) == xs, // List
      fs.extract(xs.toVector).featureValues[Seq[Double], Double].map(_.head) == xs.toVector,
      fs.extract(xs.toBuffer).featureValues[Seq[Double], Double].map(_.head) == xs)
  }
