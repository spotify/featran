...
object CrossingSpec extends Properties("CrossingSpec") {
  ...
  private def denseNames(a: String, b: String) = Seq(
    s"${a}_0", s"${a}_1", s"${b}_0", s"${b}_1", s"${b}_2",
    s"cross_${a}_0_x_${b}_0", s"cross_${a}_0_x_${b}_1", s"cross_${a}_0_x_${b}_2",
    s"cross_${a}_1_x_${b}_0", s"cross_${a}_1_x_${b}_1", s"cross_${a}_1_x_${b}_2")
  ...
  property("dense multi") = Prop.forAll { xs: List[A] =>
    ...
    val expected = xs.map(x => Seq(denseValues(x, _ * _), denseValues(x, _ + _)))
    Prop.all(
  ...
  private def sparseNames(a: String, b: String, xs: List[B]) = {
    ...
    n1 ++ n2 ++ (for (a <- n1; b <- n2) yield Crossings.name(a, b))
  }
  private def sparseValues(a: String, b: String, v: B, f: (Double, Double) => Double) = {
    import scala.collection.breakOut
    val m1: Map[String, Double] = flatten(v._1).map(s => (a + "_" + s, 1.0))(breakOut)
    val m2: Map[String, Double] = flatten(v._2).map(s => (b + "_" + s, 1.0))(breakOut)
